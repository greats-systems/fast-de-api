...
export class AuthService {
  ...
  async register(createUserDTO: CreateUserDTO) {
    ...
    console.log('newLogin')
    var newLogin = await this.loginUser(loginData)
    console.log(newLogin)
    let response = {
      user,
      newLogin,
  ...
  async validateUser(emai: string, pin: string): Promise<any> {
    console.log('getting User')
    const user = await this.usersService.findOneByEmail(emai);
    console.log(user)
    if (user) {
      console.log('User found')
      console.log(user)
