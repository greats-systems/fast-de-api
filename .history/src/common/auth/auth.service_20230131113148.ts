  ...
  async loginUserPhone(loginUserInput: LoginMobileUserDTO) {
    ...
    if (!user) {
      ...
      let response = {
        status: 201,
        data : null,
        err : `${user.role.charAt(0).toUpperCase() + user.role.slice(1)} 0${user.phone} not ${loginUserInput.app}, Download Fast-De ${user.role.charAt(0).toUpperCase() + loginUserInput.app.slice(1)} App `
      }
      return response;
    }
  }
