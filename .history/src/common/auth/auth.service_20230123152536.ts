...
export class AuthService {
  ...
  async decodeUserToken(token: string): Promise<import { User } from './entities/user.entity';
  > {
    const user0 : User = this.jwtTokenService.decode(token);

    if (user0) {
    const user = await this.usersService.findOneByPhone(user0.phone);
