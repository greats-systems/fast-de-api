...
export class AuthService {
  ...
  async loginUserPhone(loginUserInput: LoginMobileUserDTO) {
    ...
    if (!user) {
      ...
      return response;
    }else if(user && user.role !=loginUserInpxut.app) {
      let response = {
        status: 201,
        data : null,
        err : `User #${user.phone} not ${loginUserInput.app}, Download ${loginUserInput.app.toUpperCase()} App `
      }
      return response;
    }
  }
  ...
  async validateUser(phone: string, pin: string, app: string): Promise<any> {
    ...
    const user = await this.usersService.findOneByxPhone(phone, app);
    console.log(user)
    if (user) {
