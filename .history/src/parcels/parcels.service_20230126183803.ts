  ...
  async acceptReject(acceptRejectParcelDto: AcceptRejectParcelDto){
    ...
    if(acceptRejectParcelDto.driverDecision === 'accept'){
      ...
      if (Expo.isExpoPushToken(pushToken)) {
        ...
        const acceptingDriver = awaithis.userService.findOneByPhone(acceptRejectParcelDto.driverPhone)
        console.log(acceptingDriver);
        await this.updateParcel(acceptRejectParcelDto.packageID,(await acceptingDriver).userId)
        let category = {
          type: 'accept',
