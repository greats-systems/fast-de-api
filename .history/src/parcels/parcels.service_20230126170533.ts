...
export class ParcelsService {
  ...
  async acceptReject(acceptRejectParcelDto: AcceptRejectParcelDto){
    ...
    if(acceptRejectParcelDto.driverDecision === 'accept'){
      ...
      const parcelOwner = this.userService.findOne((await parcel).packageOwnerID)
      console.log('driver');
    ...
    if(acceptRejectParcelDto.driverDecision === 'reject'){
      let driverID = acceptRejectParcelDto.driverID
      let rejectedDriverIds : Array<{rejectedDriverId: string}> = Array(
        { "rejectedDriverId": 0, "rejectedDriverId": "Available" },
        { "id": 1, "name": "Ready" },
        { "id": 2, "name": "Started" }
    );
