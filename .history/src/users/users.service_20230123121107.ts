...
export class UsersService {
  ...
  async create(createUser: CreateMobileUserDTO): Promise<User> {
    const saltOrRounds = 10;
    const password = createUser.password;
    createUser.password = await bcrypt.hash(password, saltOrRounds);
    const userSchema = this.userRepository.create(createUser);
